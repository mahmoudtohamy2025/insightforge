...
const LANDING_VARIANT = "gulf-brand" as const;
...
// P1.2 — Six FAQs answering the predictable GCC brand-manager objections.
// Order is intentional: regional depth → cultural-context proof → vs global tools →
// data residency/compliance → agency white-label → AI output detail.
const FAQS: { q: string; a: string }[] = [
  {
    q: "Does it really understand Khaleeji vs Levantine vs Egyptian dialect nuances?",
    a: "Yes. Each AI twin's persona prompt includes an explicit dialect layer (Khaleeji for KSA/UAE/Kuwait/Qatar/Bahrain; Levantine for Jordan/Lebanon/Syria; Egyptian for Egypt). Responses are generated in the appropriate dialect with regional idiom. We use Gemini 2.5's multilingual capacity, calibrated by a 6-layer prompt structure that bakes cultural context (Wasta, family hierarchy, modesty norms) into every response, not just translation.",
  },
  {
    q: "How does it model Ramadan timing, Halal context, and Friday / Jumu'ah behavior?",
    a: "The temporal layer in the persona prompt is Hijri-aware. During Ramadan, AI twins shift their consumption patterns automatically — Iftar timing, late-night media engagement, charitable-giving priorities, Eid pre-shopping. Friday responses factor in Jumu'ah prayer + family gathering norms. December responses anchor to White Friday rather than Black Friday. All baked into the system — you don't have to prompt for it.",
  },
  {
    q: "How does this compare to global tools (Synthetic Users, Aaru, Listen Labs) or regional agencies (Edelman MENA, Insightful)?",
    a: "Global tools are trained on Western-defaulted data; they generate 'Sarah from New York with an Arabic name' personas that miss every regional nuance. Regional agencies do high-quality work but operate on 6-week timelines with five-figure budgets per study. InsightForge is the only AI-twin product purpose-built for the Gulf — 48-hour turnaround, sub-$500/mo pricing, and outputs that read like your real consumers actually do.",
  },
  {
    q: "Is data hosted in the GCC region for PDPL / NCA compliance?",
    a: "Data residency is configurable on the Enterprise tier — we deploy to Saudi-region or UAE-region data centers on request, with PDPL (Saudi) and ADGM / DIFC (UAE) compliance documentation provided. Default deployment is EU (GDPR-compliant). For most agency and brand-team use cases, EU is fine; if your client procurement requires in-region hosting, mention it on the demo call.",
  },
  {
    q: "Can my agency white-label reports for client deliverables?",
    a: "Yes. White-label is on the Enterprise tier — custom logo, custom domain (yourbrand.insightforge.io), PDF exports with your agency header. Useful for client deliverables where the AI provenance is implementation detail and the agency owns the brand surface. Setup takes ~24 hours after contract signature.",
  },
  {
    q: "What does the AI actually return per simulation?",
    a: "Structured data, not just paragraphs. Per persona response: (1) in-character response text in the appropriate dialect, (2) sentiment score (-1 to +1), (3) confidence score (0 to 1) based on persona-stimulus match, (4) purchase-intent label across 5 levels, (5) emotional reaction across 6 levels, (6) 3–5 extracted themes. Export the full study as a stakeholder-ready PDF — useful for both internal review and client presentations.",
  },
];
...
const Landing = () => {
  ...
  // P0.1 — Fire a page-view event on mount so the gulf-brand landing arm is measurable.
  useEffect(() => {
    trackPageView("/");
