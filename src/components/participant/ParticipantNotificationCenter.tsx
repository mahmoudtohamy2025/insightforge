...
export function ParticipantNotificationCenter() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`participant_notifications:${user.id}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "participant_notifications", filter: `user_id=eq.${user.id}` },
        () => fetchNotifications()
      )
      .subscribe();
  ...
  const markAsRead = async (n: Notification) => {
    ...
    await supabase.from("participant_notifications").update({ is_read: true }).eq("id", n.id);
  ...
  const markAllAsRead = async () => {
    ...
    await supabase.from("participant_notifications").update({ is_read: true }).eq("user_id", user.id).eq("is_read", false);
  ...
  const deleteNotification = async (id: string, e: React.MouseEvent) => {
    ...
    await supabase.from("participant_notifications").delete().eq("id", id);
