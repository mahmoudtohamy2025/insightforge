...
17. **Sample size: one twin per segment, vs. a positioning that says 50 (found 2026-06-12).** `simulate-focus-group` runs exactly one Gemini persona per segment per round (`segments.map`, `twin_index: i`), so a "focus group" is 2–5 simulated customers — while the locked positioning sentence promises "asking 50 AI-simulated customers" (§1). Related: `simulate-ab-test` declares its winner by a 0.05 average-sentiment margin over n = 2–5 single responses with no repeat sampling — the landing's "statistical preference scores" copy was reworded honestly (2026-06-12), but the underlying small-n remains. **The one engine investment this implies: multi-twin sampling per segment** (N twins drawn per segment per round), which would simultaneously make the "50 customers" sentence true, give A/B results a defensible n, and move brand-manager credibility (see `docs/AUDIENCE_MAP.md`). Needs scoping — token cost scales linearly with N. Until then, don't use "50" or "statistical" in any external material.
