...
| Real research | `projects`, `surveys`, `survey_questions`, `survey_responses`, `sessions`, `session_participants`, `session_transcripts` (FTS), `session_themes` (FTS), `session_notes` (FTS), `session_probes`, `session_media`, `insight_patterns` (FTS), `synthesis_runs`, `requirements` (+comments/votes) | **Naming trap: `sessions` = human research sessions; `simulations` = AI twin runs.** |
...
|---|---|---|
| Types/lint | ⚠️ The root `tsconfig.json` is solution-style (`files: []`), so `npx tsc --noEmit` — what the pre-push gate runs — **checks zero files** (known issue #13). The real check, `tsc -p tsconfig.app.json --noEmit`, has 7 pre-existing errors in 2 files. eslint 0 errors (advisory). `noImplicitAny`/`strictNullChecks` are off — TS is partial. |
...
13. **The pre-push type-check gate is a no-op (verified 2026-06-10).** `.githooks/pre-push` runs `npx tsc --noEmit`, which resolves the root solution-style `tsconfig.json` (`files: []` + references) and checks **zero files** — every historical "tsc clean" via that path was vacuous. The real check (`tsc -p tsconfig.app.json --noEmit`) reports 7 pre-existing errors in `src/components/insights/ResearchPatternsTab.tsx` (queries a `pattern_snapshots` table that exists in **no migration** — possible live-DB drift or dead feature code) and `src/services/sessionService.ts` (selects `scheduled_at`; schema has `scheduled_date`/`scheduled_time`) — both smell like real runtime bugs, not just stale generated types. **Fix:** investigate the two files against the live DB, regenerate types (`npm run sync-types`), then point the hook at `tsc -b --noEmit`. The vitest half of the gate is genuine and runs the parity/claims tripwires.
14. **Participant "Request Data Erasure" button is dead** (`src/pages/participant/ParticipantProfile.tsx:231-236`) — copy promises erasure on request but the button has no handler; only the researcher-side flow calls `erase-participant`. Wire it or reword.
15. **`data_residency` renders a fabricated "mena" fallback** when the column is null (`WorkspaceTab.tsx:313`, `AdminTenants.tsx:253`, `AdminTenantDetail.tsx:229`) — render "—" instead, and verify stored values match the project's real region.
